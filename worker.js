(function () {
    ...
    var O$1;!function(O){O[O.PROTOCOL_VERSION=0]="PROTOCOL_VERSION",O[O.PROTOCOL_MIN_VERSION=0]="PROTOCOL_MIN_VERSION",O[O.SETUP_MESSAGES_BUFFER_SIZE=10]="SETUP_MESSAGES_BUFFER_SIZE";}(O$1||(O$1={}));

    var _$1;!function(_){_.UNKNOWN="__unknown",_.INTERNAL="__internal",_.EVALUATOR="__evaluator",_.EVALUATOR_SYNTAX="__evaluator_syntax",_.EVALUATOR_TYPE="__evaluator_type",_.EVALUATOR_RUNTIME="__evaluator_runtime";}(_$1||(_$1={}));
    ...
    class o extends Error{name="ConductorError";errorType=_$1.UNKNOWN;constructor(r){super(r);}}

    let s$1 = class s extends o{name="ConductorInternalError";errorType=_$1.INTERNAL;constructor(r){super(r);}};

    let e$1 = class e{name;t;i=new Set;h=true;o=[];send(s,t){this.l(),this.t.postMessage(s,t??[]);}subscribe(s){if(this.l(),this.i.add(s),this.o){for(const t of this.o)s(t);delete this.o;}}unsubscribe(s){this.l(),this.i.delete(s);}close(){this.l(),this.h=false,this.t?.close();}l(){if(!this.h)throw new s$1(`Channel ${this.name} has been closed`)}_(s){if(this.l(),this.o){if(this.o.length>=O$1.SETUP_MESSAGES_BUFFER_SIZE)return console.warn("Channel buffer full; message dropped (no subscribers on channel)",s);this.o.push(s);}else for(const t of this.i)t(s);}listenToPort(s){s.addEventListener("message",(s=>this._(s.data))),s.start();}replacePort(s){this.l(),this.t?.close(),this.t=s,this.listenToPort(s);}constructor(s,t){this.name=s,this.replacePort(t);}};class h{name;u;p=new s$2;async receive(){return this.p.pop()}tryReceive(){return this.p.tryPop()}send(s,t){this.u.send(s,t);}close(){this.u.unsubscribe(this.p.push);}constructor(s){this.name=s.name,this.u=s,this.u.subscribe(this.p.push);}}let n$1 = class n{m=true;C;v;P=new Map;M=new Map;j=[];A(s){const{port1:t,port2:i}=new MessageChannel,h=new e$1(s,t);this.C.postMessage([s,i],[i]),this.P.set(s,h);}l(){if(!this.m)throw new s$1("Conduit already terminated")}registerPlugin(s,...i){this.l();const e=[];for(const t of s.channelAttach)this.P.has(t)||this.A(t),e.push(this.P.get(t));const h=new s(this,e,...i);if(void 0!==h.id){if(this.M.has(h.id))throw new s$1(`Plugin ${h.id} already registered`);this.M.set(h.id,h);}return this.j.push(h),h}unregisterPlugin(s){this.l();let t=0;for(let i=0;i<this.j.length;++i)this.j[t]===s&&++t,this.j[i]=this.j[i+t];for(let s=this.j.length-1,i=this.j.length-t;s>=i;--s)delete this.j[s];s.id&&this.M.delete(s.id),s.destroy?.();}lookupPlugin(s){if(this.l(),!this.M.has(s))throw new s$1(`Plugin ${s} not registered`);return this.M.get(s)}terminate(){this.l();for(const s of this.j)s.destroy?.();this.C.terminate?.(),this.m=false;}$(s){const[t,i]=s;if(this.P.has(t)){const s=this.P.get(t);this.v?s.listenToPort(i):s.replacePort(i);}else {const s=new e$1(t,i);this.P.set(t,s);}}constructor(s,t=false){this.C=s,s.addEventListener("message",(s=>this.$(s.data))),this.v=t;}};

    async function t$2(t){return (await import(/* webpackIgnore: true */t)).plugin}

    async function r$2(r){return await t$2(r)}

    let t$1 = class t{type=R.CALL;data;constructor(s,t,r){this.data={fn:s,args:t,invokeId:r};}};let r$1 = class r{type=R.RETURN_ERR;data;constructor(s,t){this.data={invokeId:s,err:t};}};let a$1 = class a{type=R.RETURN;data;constructor(s,t){this.data={invokeId:s,res:t};}};

    function s(s,o){const c=[];let a=0;return s.subscribe((async n=>{switch(n.type){case R.CALL:{const{fn:r,args:c,invokeId:a}=n.data;try{const t=await o[r](...c);a>0&&s.send(new a$1(a,t));}catch(e){a>0&&s.send(new r$1(a,e));}break}case R.RETURN:{const{invokeId:e,res:t}=n.data;c[e]?.[0]?.(t),delete c[e];break}case R.RETURN_ERR:{const{invokeId:e,err:t}=n.data;c[e]?.[1]?.(t),delete c[e];break}}})),new Proxy({},{get(e,t,r){const o=Reflect.get(e,t,r);if(o)return o;const i="string"==typeof t&&"$"===t.charAt(0)?(...e)=>{s.send(new t$1(t,e,0));}:(...e)=>{const r=++a;return s.send(new t$1(t,e,r)),new Promise(((e,t)=>{c[r]=[e,t];}))};return Reflect.set(e,t,i,r),i}})}

    var _,n;!function(_){_.CHUNK="__chunk",_.FILE="__file_rpc",_.SERVICE="__service",_.STANDARD_IO="__stdio",_.RESULT="__result",_.ERROR="__error",_.STATUS="__status",_.PLUGIN="__plugin";}(_||(_={})),function(_){_.HOST_MAIN="__host_main",_.RUNNER_MAIN="__runner_main";}(n||(n={}));
    ...
    class t{type=a.ABORT;data;constructor(s){this.data={minVersion:s};}}class e{type=a.HELLO;data={version:O$1.PROTOCOL_VERSION}}
    ...
    */
    ...
    function typeTranslator$1(type) {
        switch (type) {
            case "bigint":
                return "int";
            case "number":
    ...
    class UnsupportedOperandTypeError extends RuntimeSourceError {
        constructor(source, node, wrongType1, wrongType2, operand) {
            ...
            let hint = "TypeError: unsupported operand type(s) for " +
                operand +
                ": '" +
                wrongType1 +
                "' and '" +
...
You called ${this.functionName}() without providing the required positional argument ${this.missingParamName}. Make sure to pass all required arguments when calling ${this.functionName}.`;
    ...
    class StepLimitExceededError extends RuntimeSourceError {
        constructor(source, node) {
            ...
            const index = node.startToken.indexInSource;
            const { lineIndex, fullLine } = getFullLine(source, index);
            const errorPos = "operator" in node ? node.operator.indexInSource - node.startToken.indexInSource : 0;
    ...
    class UnboundLocalError extends RuntimeSourceError {
    ...
    class NameError extends RuntimeSourceError {
        constructor(source, name, node) {
            super(node);
            this.type = ErrorType.TYPE;
            const { lineIndex, fullLine } = getFullLine(source, node.startToken.indexInSource);
    ...
    class BuiltinReassignmentError extends RuntimeSourceError {
        constructor(source, name, node) {
            super(node);
            this.type = ErrorType.TYPE;
            const { lineIndex, fullLine } = getFullLine(source, node.startToken.indexInSource);
    ...
    */
    ...
    function createOutputStream(conductor) {
    ...
    function createErrorStream(conductor) {
        const stream = new WritableStream({
            write: chunk => {
    ...
    const createInputStream = (conductor) => {
    ...
    const displayError = async (context, error, type) => {
    ...
    const destroyStreams = async (context) => {
        if (context.streams.initialised) {
    ...
    */
    ...
    const createEnvironment = (context, closure, args, callExpression) => {
        const environment = {
            name: closure.node.constructor.name === "FunctionDef"
    ...
    function isInstr(item) {
        return item.instrType !== undefined;
    }
        ...
        switch (operator) {
            ...
            case TokenType.MINUS:
                switch (value.type) {
                    ...
                    default:
                        handleRuntimeError(context, new UnsupportedOperandTypeError(code, command, value.type, "", operatorTranslator(operator)));
    ...
    function evaluateBinaryExpression(code, command, context, operator, left, right) {
        ...
        if (left.type === "complex" || right.type === "complex") {
            if ((right.type !== "complex" &&
                ...
                (left.type !== "complex" &&
                    ...
                    left.type !== "bigint" &&
                    left.type !== "string")) {
                handleRuntimeError(context, new UnsupportedOperandTypeError(code, command, left.type, right.type, operatorTranslator(operator)));
                return {
                    type: "error",
        ...
        if ((left.type !== "bool" && left.type !== "number" && left.type !== "bigint") ||
    ...
    function evaluateBoolExpression(code, command, context, operator, left, right) {
        if (operator === TokenType.OR) {
            ...
            handleRuntimeError(context, new UnsupportedOperandTypeError(code, command, typeTranslator$1(left.type), typeTranslator$1(right.type), operatorTranslator(operator)));
     ...
     */
    async function evaluate(code, program, context, options = {}) {
        source = code;
        try {
            ...
            const result = await runCSEMachine$1(code, context, context.control, context.stash, options.envSteps, options.stepLimit, options.isPrelude);
            return result;
        }
        catch (error) {
            return await displayError(context, error, "__evaluator_runtime" /* ErrorType.EVALUATOR_RUNTIME */);
        }
        finally {
     ...
     */
    async function runCSEMachine$1(code, context, control, stash, envSteps, stepLimit, isPrelude = false) {
     ...
     */
    async function* generateCSEMachineStateStream(code, context, control, stash, _envSteps, stepLimit, isPrelude = false) {
        ...
        while (command) {
            ...
            if (!isPrelude && steps === stepLimit) {
                handleRuntimeError(context, new StepLimitExceededError(source, command));
            }
    ...
    const cmdEvaluators = {
         ...
         * AST Nodes
         */
        FileInput: function (_code, command, context, control, _stash, isPrelude) {
            const node = command;
        ...
        SimpleExpr: function (_code, command, _context, control, _stash, _isPrelude) {
            const simpleExpr = command;
            control.push(simpleExpr.expression);
        },
        Literal: function (_code, command, _context, _control, stash, _isPrelude) {
        ...
        BigIntLiteral: function (_code, command, _context, _control, stash, _isPrelude) {
            const literal = command;
            stash.push({ type: "bigint", value: BigInt(literal.value) });
        },
        Unary: function (_code, command, _context, control, _stash, _isPrelude) {
            const unary = command;
            const op_instr = unOpInstr(unary.operator.type, unary);
            control.push(op_instr);
            control.push(unary.right);
        },
        Binary: function (_code, command, _context, control, _stash, _isPrelude) {
        ...
        BoolOp: function (_code, command, _context, control, _stash, _isPrelude) {
        ...
        Grouping: function (_code, command, _context, control, _stash, _isPrelude) {
            const groupingNode = command;
            control.push(groupingNode.expression);
        },
        Complex: function (_code, command, _context, _control, stash, _isPrelude) {
            const complexNode = command;
            stash.push({ type: "complex", value: complexNode.value });
        },
        None: function (_code, _command, _context, _control, stash, _isPrelude) {
            stash.push({ type: "none" });
        },
        Variable: function (code, command, context, _control, stash, _isPrelude) {
        ...
        Compare: function (_code, command, _context, control, _stash, _isPrelude) {
        ...
        Assign: function (_code, command, _context, control, _stash, _isPrelude) {
            const assignNode = command;
            if (assignNode.target instanceof ExprNS.Subscript) {
                throw new Error("Subscript assignment is not yet supported");
            }
            const assmtInstr$1 = assmtInstr(assignNode.target.name.lexeme, false, true, assignNode);
            control.push(assmtInstr$1);
            control.push(assignNode.value);
        },
        Call: function (_code, command, _context, control, _stash, _isPrelude) {
        ...
        FunctionDef: function (_code, command, context, _control, _stash, _isPrelude) {
            const functionDefNode = command;
            const localVariables = scanForAssignments(functionDefNode.body);
            const closure = Closure.makeFromFunctionDef(functionDefNode, currentEnvironment(context), context, localVariables);
            pyDefineVariable(context, functionDefNode.name.lexeme, { type: "closure", closure });
        },
        Lambda: function (_code, command, context, _control, stash, _isPrelude) {
            const lambdaNode = command;
            const localVariables = scanForAssignments(lambdaNode.body);
            const closure = Closure.makeFromLambda(lambdaNode, currentEnvironment(context), context, localVariables);
            stash.push({ type: "closure", closure });
        },
        Return: function (_code, command, _context, control, stash, _isPrelude) {
        ...
        If: function (_code, command, _context, control, _stash, _isPrelude) {
        ...
        Ternary: function (_code, command, _context, control, _stash, _isPrelude) {
         ...
         * Instructions
         */
        [InstrType.RESET]: function (_code, _command, context, _control, _stash, _isPrelude) {
            popEnvironment(context);
        },
        [InstrType.ASSIGNMENT]: function (code, command, context, _control, stash, _isPrelude) {
            const instr = command;
            const value = stash.pop();
            if (value) {
                if (builtIns.has(instr.symbol)) {
                    throw new BuiltinReassignmentError(code, instr.symbol, instr.srcNode);
                }
                pyDefineVariable(context, instr.symbol, value);
            }
        },
        [InstrType.UNARY_OP]: function (code, command, context, _control, stash, _isPrelude) {
            const instr = command;
        ...
        [InstrType.BINARY_OP]: function (code, command, context, _control, stash, _isPrelude) {
            const instr = command;
        ...
        [InstrType.BOOL_OP]: function (code, command, context, _control, stash, _isPrelude) {
            const instr = command;
            const right = stash.pop();
            const left = stash.pop();
            if (left && right) {
        ...
        [InstrType.POP]: function (_code, _command, _context, _control, stash, _isPrelude) {
            stash.pop();
        },
        [InstrType.APPLICATION]: async function (code, command, context, control, stash, _isPrelude) {
            const instr = command;
        ...
        [InstrType.BRANCH]: function (_code, command, _context, control, stash, _isPrelude) {
            const instr = command;
        ...
        [InstrType.ENVIRONMENT]: function (_code, command, context, _control, _stash, _isPrelude) {
            while (currentEnvironment(context).id !== command.env.id) {
                popEnvironment(context);
            }
        },
        [InstrType.END_OF_FUNCTION_BODY]: function (_code, _command, _context, _control, stash, _isPrelude) {
            stash.push({ type: "none" });
        },
    };
    ...
    function runPyAST(code, chapter = 4, doValidate = false) {
        const script = code + "\n";
    ...
    async function runCSEMachine(code, program, context, options = {}) {
        const result = await evaluate(code, program, context, options);
    ...
    class PyEvaluator extends r {
        ...
        async evaluateChunk(chunk) {
            try {
                ...
                await runInContext(chunk, // Code
                this.context, this.options);
            }
            catch (error) {
                if (error instanceof SyntaxError) {
                    await displayError(this.context, error, "__evaluator_syntax" /* ErrorType.EVALUATOR_SYNTAX */);
                    return;
                }
                await displayError(this.context, error, "__internal" /* ErrorType.INTERNAL */);
            }
            finally {
