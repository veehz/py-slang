...
async function loadPyodideGeneric() {
    ...
    const indexURL = IN_NODE
        ? await ensureLocalPyodideAssets(cdnBase)
        : cdnBase;
...
const TORCH_SCRIPT_URL = "https://veehz.github.io/torch/build/torch.min.js";
/**
 * Loads the torch.js library into the global scope, then runs bridge.py
 * inside Pyodide to set up the Python-side `torch` module.
 *
 * After this call, `pyodide.globals.get("torch")` is the usable torch module.
 */
async function loadTorch(pyodide) {
    // --- Load torch.min.js into globalThis ---
    if (typeof document !== "undefined") {
        await new Promise((resolve, reject) => {
            const script = document.createElement("script");
            script.src = TORCH_SCRIPT_URL;
            script.onload = () => resolve();
            script.onerror = () => reject(new Error(`Failed to load script: ${TORCH_SCRIPT_URL}`));
            document.head.appendChild(script);
        });
    }
    else if (typeof globalThis.importScripts === "function") {
        globalThis.importScripts(TORCH_SCRIPT_URL);
    }
    else {
        throw new Error("Neither 'document' nor 'importScripts' available to load torch.");
    }
    const torchGlobal = globalThis.torch;
    if (!torchGlobal) {
        throw new Error("torch global not found after loading scripts (check window.torch or globalThis.torch)");
    }
    // Expose the JS torch object to Python.
    pyodide.globals.set("js_torch", torchGlobal);
    // --- Run the Python bridge that wraps js_torch as a proper `torch` module ---
...
class BuiltInFunctions {
    ...
    static input(_args, _source, _command, _context) {
        // TODO: : call conductor to receive user input
        return { type: "string", value: "" };
    }
    static print(args, _source, _command, context) {
 ...
 *
 * Uses py-slang's parser to produce an AST, then walks FromImport nodes
 * to find torch-related imports — replacing the regex-based approach used
 * in sa-conductor-py-torch.
 */
/**
 * Parses the source code using py-slang's parser and extracts all
 * FromImport statements whose root module is "torch".
 */
function detectTorchImports(source) {
    let ast;
    try {
        ast = parse(source);
    }
    catch {
        // If parsing fails, fall back to no detection — let pyodide
        // report the real syntax error later.
        return [];
    }
    const torchImports = [];
    for (const stmt of ast.statements) {
        ...
        torchImports.push({
            ...
            startLine: stmt.startToken.line,
 ...
 */
function getNonTorchImportRoots(source) {
    let ast;
    try {
        ast = parse(source);
    }
...
function rewriteTorchImports(source) {
    ...
    for (let i = imports.length - 1; i >= 0; i--) {
        ...
        const startIdx = imp.startLine - 1;
        const endIdx = imp.endLine - 1;
...
exports.PyodideEvaluator = PyodideEvaluator;
exports.detectTorchImports = detectTorchImports;
exports.getNonTorchImportRoots = getNonTorchImportRoots;
exports.rewriteTorchImports = rewriteTorchImports;
//# sourceMappingURL=pyodide-evaluator.cjs.map
